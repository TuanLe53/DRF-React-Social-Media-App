...
export default function PostDetail({open, post}) {
    ...
    const fetchData = async () => {
        let res = await fetch(ServerUrl.API_URL + `post/comment/${post.id}/`, {
            method: "GET",
    ...
    const fetchLikeCount = () => {
        fetch(ServerUrl.API_URL + `post/like-count/${post.id}/`)
            .then(res => res.json())
            .then(res => {
                setLikeCount(res)
            })
        ...
        <Grid className="post-container">
            <Row>
                <Col xs={12}>
                    <Carousel shape="bar">
                        {post.images.map((img) => (
                            <img key={img.id} src={ServerUrl.BASE_URL + img.image}/>
                        ))}
                    </Carousel>
                </Col>
                <Col xs={11}>
                    <Container>
                        <Header>
                            <Link to={`/profile/${post.created_by}`} state={{ user: post.created_by }}>
                                <Stack spacing={5}>
                                    {profile ? <Avatar src={ServerUrl.BASE_URL + profile.avatar_img} circle /> : <Loader /> }
