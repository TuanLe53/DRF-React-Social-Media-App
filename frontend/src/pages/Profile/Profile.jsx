...
import InboxBtn from "../../components/profile/InboxBtn"
...
export default function Profile() {
    ...
    const updateAvatar = async (e) => {
        ...
        let res = await fetch(ServerUrl.API_URL + `profile/${user.username}/update-avatar/`, {
            method: 'PUT',
            headers: {
                ...
                <>
                    <Container className="profile">
                        <Header>
                            <Grid fluid>
                                <Row>
                                    <Col>
                                        <Avatar src={ServerUrl.BASE_URL + profile.avatar_img} round={true} size={150} onClick={() => setOpen(true)} />
                                    </Col>
                                    <Col>
                                        <Stack spacing={10}>
                                            <p>{user_profile}</p>
                    ...
                    {user.username === user_profile ?
                            <Modal open={open} onClose={() => setOpen(false)} className="avatar-box-update">
                                ...
                                <Modal.Body>
                                    <Container>
                                        <img src={ServerUrl.BASE_URL + profile.avatar_img} />
                                    </Container>
                        ...
                        <Modal open={open} onClose={() => setOpen(false)} className="avatar-box">
                            <Avatar src={ServerUrl.BASE_URL + profile.avatar_img} round={true} size={500} />
                        </Modal>
                    }
...
function EditProfileForm({user_profile, authTokens, setIsEdit, setProfile}) {
    ...
    const handleSubmit = async (e) => {
        ...
        let res = await fetch(ServerUrl.API_URL + `profile/${user_profile}/edit-profile/`, {
            method: "PATCH",
            headers: {
                'Authorization': 'Bearer ' + String(authTokens.access),
            },
...
function Followers({user_profile, authTokens}) {
    ...
    useEffect(() => {
        fetch(ServerUrl.API_URL + `profile/${user_profile}/follower/`, {
            method: "GET",
            headers: {
                'Authorization': 'Bearer ' + String(authTokens.access)
            }
...
function Followings({user_profile, authTokens}) {
    ...
    useEffect(() => {
        fetch(ServerUrl.API_URL + `profile/${user_profile}/following/`, {
            method: "GET",
            headers: {
                'Authorization': 'Bearer ' + String(authTokens.access)
            }
...
function FollowerUser({user, authTokens}) {
    ...
    return (
        <Link to={`/profile/${user.follower}`} state={{ user: user.follower }} >
            <Stack spacing={10}>
                {followUser ? <Avatar src={ServerUrl.BASE_URL + followUser.avatar_img} round={true} size={40} /> : <Loader /> }
                <p>{user.follower}</p>
